...
@IonicPage()
@Component({
  selector: 'page-callsfeedback',
  templateUrl: 'callsfeedback.html',
})
export class callfeedbackpage {
  ...
  getFeedbackCalls() {
    ...
    this.authService.postData(arr_data, 'Call_inspection/getfeedbackcalls').then((result) => {
  ...
  openupdatefeedback(item = [] as any) {
    ...
    const myModal: Modal = this.modal.create('callsfeedbackupdatepage', { data: myModalData }, myModalOptions);

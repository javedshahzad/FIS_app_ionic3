...
@IonicPage()
@Component({
  selector: 'page-taskfeedbackcompletedlist',
  templateUrl: 'taskfeedbackcompletedlist.html',
})
export class taskCompletedfeedbackpage {
  ...
  SearchcallManagement() {
    ...
    if (item != '') {
      if (isNaN(call_id)) {
        this.Callprocurementvalue = this.Callprocurement.filter(call => (call.TITLE ? call.TITLE.includes(item) : false));
      } else if (!isNaN(call_id)) {
        this.Callprocurementvalue = this.Callprocurement.filter(call => call.TASK_ID === call_id);
      }
    } else {

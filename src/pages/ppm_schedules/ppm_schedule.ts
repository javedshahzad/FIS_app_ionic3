...
@IonicPage()
@Component({
  selector: 'page-ppmschedule',
  templateUrl: 'ppm_schedule.html',
})

export class ppmscheduledetails {
  ...
  searchData = {"search_value": ""};
    ...
    constructor(public navCtrl: NavController, public navParams: NavParams, private formBuilder: FormBuilder,
        ...
        public constant: Constant,public platform: Platform ,private file: File,private fileOpener: FileOpener) {
  ...
  ngOnInit() {
    ...
    this.Schedule = this.Scheduledata.ScheduleList;
    this.searchScheduleList =this.Schedule;
    if(this.Scheduledata.STATUS == -1){
      this.status_name ='Processsing'
    }else if(this.Scheduledata.STATUS == 1){
      this.status_name ='Finished'
    }else{
...
confirmupdate(ASSET_DETAILS_PM_ID:any){
  Swal.fire({
    ...
    if (result.value) {
     ...
     //   this.navCtrl.setRoot(PpmscheduleoptionModule);
     this.closeModal();
...
GetScheduleList(ScheduleNo){
  ...
  this.presentLoadingDefault(true);
  this.authService.getData({},url).then((result:any) => {
    let Schedule = result;
    this.Schedule = Schedule.filter(x => x.STATUS == this.Scheduledata.STATUS);
    this.searchScheduleList =this.Schedule;
    this.Scheduledata.ScheduleList =  this.Schedule;
...
Search(){
  ...
  if(value!=''){
    let Schedule = this.searchScheduleList.filter(item => this.filter(item));

    let schedule_asset_name = this.searchScheduleList.filter((item) =>{
      let _val = item['ASSETDETAIL_NAME'] ? item['ASSETDETAIL_NAME'].toString().toUpperCase() :'';
      return _val.includes(value.toUpperCase())
     });
     this.Schedule = [...Schedule,...schedule_asset_name];
  }else{
    this.Schedule = this.searchScheduleList;
  }
}
...
filter(item){
 ...
 // let _ASM_val = item['ASSETDETAIL_NAME'] ? item['ASSETDETAIL_NAME'].toString() :''; ||_ASM_val.includes(_ASM_val)
  let _CODE_val = item['CODE'] ? item['CODE'].toString().toLowerCase() :'';
  let _UNIT_val = item['UNIT'] ? item['UNIT'].toString().toLowerCase() :'';
  return (_SCHEDULE_val.includes(_val)  ||  _CODE_val.includes(_val) || _UNIT_val.includes(_val) );
}

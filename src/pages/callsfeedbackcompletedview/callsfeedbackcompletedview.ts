...
@IonicPage()
@Component({
  selector: 'page-callsfeedbackcompletedview',
  templateUrl: 'callsfeedbackcompletedview.html',
})
export class callsfeedbackcompletedviewepage {
  ...
  getFeedbackCallsFiles() {
    ...
    this.authService.postData(arr_data, 'Call_inspection/getFeedbackFilesAll').then((result) => {
      ...
      this.base64Image = this.getImage(7);
      this.signatureImage = this.getImage(14);
  ...
  getImage(type:any) {
    let objFile = this.callfilelistAll.filter(o => o.REFERENCE_TYPE === type);

...
@IonicPage()
@Component({
  selector: 'page-userresorcetasklist',
  templateUrl: 'userresorcetasklist.html',
})
 
export class userresorcetasklistPage {
    ...
    this.authService.getData({}, `task/resourcemanager/${this.user.UserInfoId}/${user_type}`).then((result) => {
        ...
        if (this.taskdetails.tasktime.length > 0) {
          ...
          if (taskTimelist.length > 0) {
            for (let k = 0; k < taskTimelist.length; k++) {
              if (taskTimelist[k].TASK_TIME) {
                let splitTime1 = taskTimelist[k].TASK_TIME.split(':');
                let work_time = parseInt(splitTime1[0]) + ':' + parseInt(splitTime1[1]) + ':' + parseInt(splitTime1[2]);
                _percent += Math.round(100 * (this.totalSeconds(work_time) / this.totalSeconds(total_time)));
              }
            }
      ...
      if (this.taskdetails.tasktime.length > 0) {
        ...
        for (let x = 0; x < this.taskdetails.tasktime.length; x++) {
          if (this.taskdetails.tasktime[x].TASK_TIME) {
  ...
  getTaskDetails(type: any, label_user_id: any, tab_type: any) {
    ...
    let params = {} as any;
    params.UserInfoId = this.modaltype[1]; //this.user.UserInfoId;

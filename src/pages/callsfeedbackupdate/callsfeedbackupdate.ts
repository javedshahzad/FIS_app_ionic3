...
@IonicPage()
@Component({
  selector: 'page-callsfeedbackupdate',
  templateUrl: 'callsfeedbackupdate.html',
})
export class callsfeedbackupdatepage {
  ...
  ionViewDidLoad() {
    ...
    this.callID = this.Data[0].feedbackcalls.CALL_LOG_ID;
  ...
  insertCustomerFeedback(){
    ...
    let scan_file_size = this.calculateImageSize(this.base64Str_scan[1]);
    ...
    if (this.jobcardno.length > 0) {
      ...
      this.authService.postData(params, 'Call_inspection/getUpdateJobCardNo').then((result) => {
        ...
        if (this.insertedValues == 100) {
          ...
          this.authService.postData(params1, 'Call_inspection/getInsertCustomerFeedbackWithLocation').then((result) => {

...
export class LoginPage {
  ...
  login() {
    ...
    if (this.userData.user_name) {
      if (this.userData.user_password) {
        this.btnLogIn = true;
        this.LogIn = 'Logging ...';
        this.authService.loginPostData(this.userData, 'account/login').then((result: any) => {
  ...
  TokenSetup() {
    ...
    this.fcm.getToken().then((token) => {
      ...
      this.authService.postData(fcmdata, 'account/fcmtokeninsert').then((result) => {
        this.presentLoadingDefault(false);
        this.insertedValues = result;
        console.log('Fcm after insert', this.insertedValues);
      }, (err) => {
        this.presentLoadingDefault(false);
        console.log(err);
        this.presentToast("Something went to wrong, please try again later");
      ...
      this.fcm.onNotification().subscribe((data) => {
        if (data.wasTapped) {
          ...
          console.log("Received in foreground,Login");
          //this.presentToast(data.body);
          if (data.trans_type == "TMS") {
